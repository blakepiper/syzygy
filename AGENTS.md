...
| `TASKS.md` | The ordered task checklist: a one-line history of every shipped milestone, then the full spec for current work — find your task here, check it off when done |
...
shipped milestones as one summary row each, current work as a full spec. Read
the milestone there before implementing.
