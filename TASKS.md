...
- [ ] M16.1a Add `src/syzygy/local_models/` as the non-UI subsystem. Define
...
- [ ] M16.1b Write an ADR covering the trust boundary, supported platform
...
- [ ] M16.1c Add a namespaced `local_model` settings section using
...
- [ ] M16.1d Define `LocalModelPaths` from `AppPaths`: Syzygy-owned runtime,
...
- [ ] M16.1e Specify the setup state machine before building the screen:
...
- [ ] M16.2a Implement platform adapters for OS/version/architecture, physical
      ...
      fallback.
- [ ] M16.2b Detect accelerators and usable memory: Apple unified memory and
...
- [ ] M16.2c Produce a plain-language assessment (`comfortable`, `possible
...
- [ ] M16.2d Implement the fit calculator as deterministic domain logic driven
...
- [ ] M16.2e Tests use captured/redacted fixtures for macOS Intel/Apple
...
- [ ] M16.3a Create a versioned, packaged model catalog separate from UI code.
...
- [ ] M16.3b Build an opt-in maintainer evaluation harness (not part of normal
...
- [ ] M16.3c Establish release gates before choosing catalog defaults: all
...
- [ ] M16.3d Populate at least three user-facing choices when evaluation
...
- [ ] M16.3e Recommendation output is explainable and stable: chosen artifact,
...
- [ ] M16.4a Probe the saved external endpoint and conventional localhost
...
- [ ] M16.4b Discover `llama-server` and the newer unified `llama` command on
...
- [ ] M16.4c Inspect a discovered server/binary without modifying it. Classify
...
- [ ] M16.4d If a compatible endpoint is already running, offer **Use this**
...
- [ ] M16.4e Tests cover PATH precedence, symlinks, spaces/non-ASCII in paths,
...
- [ ] M16.5a Select a runtime build from an allowlisted platform/backend
      ...
      beginner path.
- [ ] M16.5b Present the source, pinned version, download size, install
      ...
      privilege elevation.
- [ ] M16.5c Implement resumable download with progress, cancellation,
      ...
      unverified file.
- [ ] M16.5d Qualify the installed runtime through M16.4's capability probe.
...
- [ ] M16.5e Tests use a fake package runner and local fake HTTP server for
...
- [ ] M16.6a Before download, show the selected model's purpose, provenance,
...
- [ ] M16.6b Download into Syzygy-owned storage with the same resumable,
...
- [ ] M16.6c Discover existing local `.gguf` files only through paths the user
...
- [ ] M16.6d Make download failures actionable: offline, authentication/gated
      ...
      or model.
- [ ] M16.6e Add **Manage local files** outside the wizard: list managed vs
...
- [ ] M16.6f Tests cover every failure above, license acceptance versioning,
...
- [ ] M16.7a Build a subprocess supervisor outside the TUI. Launch a qualified
...
- [ ] M16.7b Define lifecycle as app-managed by default: start on demand before
...
- [ ] M16.7c Poll readiness with startup phases and a hardware/model-dependent
...
- [ ] M16.7d Persist enough process identity to recover after an app crash, but
...
- [ ] M16.7e Add health checks and one bounded automatic restart before a local
...
- [ ] M16.7f Tests use fake subprocesses and servers for exact argv/no shell,
...
- [ ] M16.8a Extend the llama.cpp probe beyond `GET /models`: verify chat
...
- [ ] M16.8b Add a no-side-effect smoke-test service that runs representative
...
- [ ] M16.8c Activate the provider atomically only after the smoke test passes.
...
- [ ] M16.8d On normal app startup, validate managed configuration cheaply.
...
- [ ] M16.8e Tests prove no provider activation on partial success, rollback to
...
- [ ] M16.9a Replace the llama.cpp row's current endpoint-only form with two
      ...
      process code.
- [ ] M16.9b Implement the state machine from M16.1 as resumable screens or one
...
- [ ] M16.9c Recommendation view offers **Recommended**, **Faster/smaller**, and
...
- [ ] M16.9d Consent/review is an exact receipt of network contacts, files,
...
- [ ] M16.9e Integrate with layout tiers and motion settings. The wizard works
...
- [ ] M16.9f Pilot tests cover fresh happy path, existing endpoint, existing
...
- [ ] M16.10a Add `syzygy model setup-local` with the same orchestrator and
      ...
      Syzygy-owned.
- [ ] M16.10b Extend `syzygy doctor` with redacted hardware/runtime/model/
...
- [ ] M16.10c Update README/user documentation with a screenshot-free beginner
...
- [ ] M16.10d Add maintainer docs for refreshing the pinned llama.cpp/runtime
...
- [ ] M16.10e Run the full repository verification from `AGENTS.md` on Python
...
- [ ] A compatible existing endpoint or binary is detected and reused without
      being modified; an incompatible one receives a precise explanation.
- [ ] Every recommendation is traceable to machine facts, artifact sizes,
      license review, and Syzygy-specific structured-output evaluation.
- [ ] Every acquired byte has prior consent and post-download integrity
      verification; every managed process is localhost-only and safely owned.
- [ ] A real reading, chart summary, and cosmos summary can use the managed
      provider after restart, while failures retain the fixed card/state and
      degrade to the established retry/fixture paths.
- [ ] Setup and verification cannot create a reading, draw a card, alter
